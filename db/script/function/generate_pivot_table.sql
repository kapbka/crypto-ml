create or replace function generate_pivot_table(
    ...
    begin
        -- drop out table if exists (drop because the set of columns could have been changed -- new intervals were added, for example)
        execute ('drop table if exists ' || quote_ident(out_table));
        ...
        for j in 1..n_clsc_cols loop
            ...
            for k in 1..n_class_cols loop
                ...
                col := col || replace(clsc_cols[(j - 1) * n_class_cols + k], '''', '');
            end loop;
            ali := '_clsc_' || j::text;
